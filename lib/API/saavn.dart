...
Future<List> topSongs() async {
  ...
  var songsListJSON =
      await http.get(Uri.parse(topSongsUrl), headers: {"Accept": "application/json"});
...
Future fetchSongDetails(songId) async {
  ...
  if (getMain[songId]["more_info"]["has_lyrics"] == "true") {
    ...
    var lyricsRes =
        await http.get(Uri.parse(lyricsUrl), headers: {"Accept": "application/json"});
    ...
    var lyricsApiRes =
        await http.get(Uri.parse(lyricsApiUrl), headers: {"Accept": "application/json"});
